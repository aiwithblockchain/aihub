...
func (h *Handler) xhsCollectNote(w http.ResponseWriter, r *http.Request) {
	log.Printf("[xhsCollectNote] received request method=%s", r.Method)
	if r.Method != http.MethodPost {
		jsonErr(w, 405, "method_not_allowed")
		return
	}
	body, err := readRawBody(r)
	if err != nil {
		log.Printf("[xhsCollectNote] failed to read body: %v", err)
		jsonErr(w, 400, err.Error())
		return
	}
...
func (h *Handler) igGetUserMedia(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		jsonErr(w, 405, "method_not_allowed")
		return
	}
...
func (h *Handler) igGetNotifications(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		jsonErr(w, 405, "method_not_allowed")
		return
	}
