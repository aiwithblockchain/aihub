...
class CommonWorkflows:
    ...
    def read_and_like_first_tweet(self, tab_id: Optional[int] = None) -> ActionResult:
        tweet = self.read.get_first_timeline_tweet(tab_id=tab_id)
        if not tweet or not tweet.id:
            raise ParseError("No tweet found in timeline")
        return self.actions.like(tweet.id, tab_id=tab_id)

    def search_and_fetch_profile(self, query: str, tab_id: Optional[int] = None) -> Optional[XUser]:
        user = self.read.search_first_user(query=query, tab_id=tab_id)
        if not user or not user.screen_name:
            return None
        return self.read.get_user(user.screen_name, tab_id=tab_id)

    def reply_to_pinned_tweet(self, username: str, text: str, tab_id: Optional[int] = None) -> ActionResult:
        tweet = self.read.get_pinned_tweet(username, tab_id=tab_id)
        if not tweet or not tweet.id:
            raise ParseError(f"No pinned tweet found for @{username}")
        return self.actions.reply(tweet.id, text)

    def analyze_tweet_and_generate_reply(self, tweet_id: str, platform: str, tab_id: Optional[int] = None) -> AIMessageResult:
        tweet = self.read.get_tweet(tweet_id, tab_id=tab_id)
        prompt = (
    ...
    def reply_to_pinned_tweet_with_ai(self, username: str, platform: str, tab_id: Optional[int] = None) -> ActionResult:
        tweet = self.read.get_pinned_tweet(username, tab_id=tab_id)
        if not tweet or not tweet.id:
            raise ParseError(f"No pinned tweet found for @{username}")
        ai_result = self.analyze_tweet_and_generate_reply(tweet.id, platform=platform, tab_id=tab_id)
        if not ai_result.content:
            raise ParseError("AI did not return reply content")
        return self.actions.reply(tweet.id, ai_result.content)
