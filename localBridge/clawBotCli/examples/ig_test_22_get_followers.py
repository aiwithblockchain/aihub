...
使用方法：
python ig_test_22_get_followers.py
"""
...
def test_get_followers():
    ...
    print("\n[步骤 2] 获取粉丝列表（第一页）...")
    try:
        result = client.ig.get_followers(user_id=user_id, count=12)
    ...
    if has_more and next_max_id:
        print(f"\n[步骤 3] 测试分页（获取下一页）...")
        try:
            result2 = client.ig.get_followers(user_id=user_id, count=12, max_id=next_max_id)
