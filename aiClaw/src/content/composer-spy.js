...
(function () {
            ...
            if (m.type === 'characterData') {
                ...
                if (parent) {
                    log('TEXT_CHANGE', {
                        ...
                        newText: m.target.data.slice(0, 100),
                    });
                }
            }
        }
    ...
    if (inputEl) {
        inputEl.addEventListener('input', (e) => {
            log('✏️ INPUT_EVENT', {
                type: e.type,
                currentText: e.target.textContent?.slice(0, 100) || e.target.value?.slice(0, 100),
            });
        });
    ...
    const snapshotInterval = setInterval(() => {
        ...
        if (snapStr !== window.__acLastSnap) {
            log('📸 SNAPSHOT', snap);
            window.__acLastSnap = snapStr;
        }
    ...
    window.__acSpyDump = () => {
        ...
        // 同时复制到剪贴板
        navigator.clipboard.writeText(output).then(() => console.log('✅ 日志已复制到剪贴板'), () => console.log('❌ 复制失败，请手动从上方复制'));
        return LOG;
    };
    window.__acSpyStop = () => {
        mo.disconnect();
